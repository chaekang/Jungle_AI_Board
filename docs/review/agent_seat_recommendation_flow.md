...
3. 평가 점수가 가장 높은 후보를 메타데이터 계산용 `winner`로 정한다.
4. winner 기준으로 공식 구역과 설명 블록을 고른다.
5. 후보별 근거 리뷰를 합친다.
6. 후보 중 동일 좌석 후기가 하나라도 없으면 `_build_candidate_comparison_abstention()`으로 비교를 보류한다.
7. 모든 후보에 직접 근거가 있을 때만 층/일반 비교 답변을 만든다.
8. `SeatRecommendationResponse`를 반환한다.

핵심 포인트:

- 이 흐름에서는 `ragStatus="skipped"`가 된다.
- “둘 중 뭐가 나아?” 질문은 RAG가 아니라 후보별 후기 비교로 답하며, 직접 근거가 없는 후보가 있으면 우열을 만들지 않는다.
...
3. 좌석번호까지 지정된 정확 리뷰가 하나라도 있으면 소표본이어도 주변 후기와 바꾸지 않고 그대로 반환한다.
4. 좌석번호가 없는 열 단위 질문에서 리뷰가 부족하면 열/좌석번호 조건을 빼고 더 넓게 검색한다.
...
- 좌석번호가 지정된 직접 후기는 소표본이어도 보존하고, 직접 후기가 없거나 열 단위 질문일 때만 검색 범위를 넓힌다.
...
Web 비교함은 `candidates` 배열로 층·구역·열·번호를 전달하고, 자유질문은 `_extract_seat_candidates()`가 텍스트에서 후보 좌석을 찾는다.
...
4. 후보별 리뷰를 점수화한다.
5. 가장 점수가 높은 후보를 메타데이터 계산용 `winner`로 선택한다.
6. 후보들의 근거 리뷰를 합친다.
7. 후보마다 동일 좌석 후기가 있으면 비교형 답변을 만들고, 하나라도 없으면 근거 부족과 부족한 좌석을 밝히며 비교를 보류한다.
8. `SeatRecommendationResponse`를 반환한다.

즉 “A와 B 중 무엇이 더 나아?”는 RAG보다 FastAPI 자체 비교 로직이 우선하며, 직접 근거가 없는 후보에 대해서는 장단점이나 승자를 만들지 않는다.
